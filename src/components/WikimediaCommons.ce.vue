...
<script setup lang="ts">
  ...
  watch(isActive, () => {
    console.log(`wc.watch.isActive: isActive=${isActive.value} qid=${qid.value} images=${allImages.value?.length || 0}`)
  ...
  onMounted(() => { 
    console.log(`wc.mounted: qid=${qid.value} isActive=${isActive.value} images=${allImages.value?.length || 0}`)
  ...
  watch(allImages, () => { 
    console.log(`wc.watch.allImages: qid=${qid.value} isActive=${isActive.value} images=${allImages.value?.length || 0}`)
  })
